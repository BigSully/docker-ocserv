...
set -x \
	...
	&& cat /etc/ocserv/groupinfo.txt >> /etc/ocserv/ocserv.conf
...
if [ ! -f /etc/ocserv/certs/server-key.pem ] || [ ! -f /etc/ocserv/certs/server-cert.pem ]; then
	...
	if [ -z "$NO_TEST_USER" ] && [ ! -f /etc/ocserv/ocpasswd ]; then
		...
		echo 'user of group Route'
		echo 'route:Route:$1$gmVgns/W$Da2MzE8F3UA3C7IGwEMwM1' >> /etc/ocserv/ocpasswd
		echo 'user of group All'
		echo 'all:All:$1$gmVgns/W$Da2MzE8F3UA3C7IGwEMwM1' >> /etc/ocserv/ocpasswd
	fi
fi
